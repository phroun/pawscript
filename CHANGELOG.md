...
- Pawgui-gtk: new GTK-based GUI alternative:
  - Using custom purfecterm-gtk terminal emulator
...
- Ported paw-gtk to Qt, with intention to support both options.
...
- CLI command line options (`--help`) for paw-gtk and paw-qt
...
- Added full CLI support to paw-gtk and paw-qt, run with --help
- Removed assumption of a single window in paw-gtk and paw-qt.
  ...
  - When all windows are closed, and all fibers have ended, the application
    will close itself.
- Added a primitive launcher to paw-gtk and paw-qt when invoked with no arguments.
  - CLI scripts will run directly inside the launcher's console window.
  - File -> New will create additional launcher instances.
- Explicit focus and autofocus of paw-gtk and paw-qt controls.
- Better default sizing and centering of paw-gtk and paw-qt console or launcher windows.
- Added automatic build process for paw-gtk and paw-qt for all platforms in Makefile
