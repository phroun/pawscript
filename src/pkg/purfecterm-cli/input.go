...
func (h *InputHandler) parseCSISequence(seq []byte) (key int, mods int, consumed int, passthrough []byte) {
	...
	// This includes uppercase A-Z, lowercase a-z, and various symbols
	...
	// CSI parameter bytes are in the range 0x30-0x3F (digits 0-9, and :;<=>?)
	// We only wait for more data if we see parameter bytes.
	// Anything else (including '-' at 0x2D) should fall through to pass-through.
	if lastByte >= 0x30 && lastByte <= 0x3F {
